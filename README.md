...
| **Vaultline: Set Embedding API Token** | Stores the embedding API credential in the OS keychain rather than settings.json. Offered automatically, with confirmation, if a token is found in settings |
| **Vaultline: Clear Embedding API Token** | Removes the stored credential. The deprecated setting, if still populated, takes over again |
...
| `maxToolRounds` | `25` | Model round trips allowed in one request while the model keeps calling tools (one round = one model call plus its tools). Was a hardcoded 8, which ordinary multi-step work exhausted — and exhausting it rendered nothing at all, because the final text was only ever captured on a round that made no tool calls. On reaching the limit the handler now warns, then makes one final call with `tools: []` — with nothing to reach for the model has to write, so a summary comes back instead of silence. No upper bound: the cancellation token is checked every round, so Stop is the real safety mechanism. **The round counter is not what actually ends a long run** — `messages` grows every round with full tool results and is never trimmed, so the context window gives out first; raising this mostly moves where that happens |
...
| `embeddingApiUrl` / `embeddingApiTimeoutMs` / `embeddingApiModel` / `embeddingApiAuthType` / `embeddingApiAuthToken` / `embeddingApiKeyHeader` | — | Only relevant when `embeddingBackend` is `"api"`. Point the URL at your own endpoint instead of the bundled server (which also disables auto-start), and set `embeddingApiFormat` to match its shape. For a **local** URL the port is a preference, not a promise: if something else holds it, the server takes the next free port within 10 and the extension follows it (see `selectPort` in `serverManager.ts`). It binds loopback only. Pointing at a **custom** endpoint (non-loopback, or any explicit `embeddingApiModel`) triggers a one-time centroid rebuild against that model — see `centroidBuilder.ts`; the result is cached under the storage dir, keyed on a hash of URL + model |
...
| `embeddingApiRequestField` / `embeddingApiResponsePath` | `"texts"` / `"embeddings"` | Only for `embeddingApiFormat: "custom"` — the request field holding the input array, and where the vectors live in the response. The path understands nesting and arrays (`embeddings`, `data[].embedding`, `result.vectors`), which is what makes `"custom"` a real option rather than a label: both presets can be written out by hand with these two, so nothing about the feature is OpenAI-specific |
| `embeddingApiFormat` / `embeddingApiEmbedPath` / `embeddingApiHealthPath` | `"vaultline"` / `""` / `"/health"` | Shape, path and health route of a custom endpoint. `"vaultline"` is the bundled server's `POST {texts} -> {embeddings}`; `"openai"` is `POST {input, model} -> {data:[{embedding, index}]}`; `"custom"` is anything else (rows are re-sorted by `index`, since trusting array order would silently pair vectors with the wrong text). An empty embed path picks the format's default (`/embed-batch` or `/v1/embeddings`). **Set `embeddingApiHealthPath` to `""` if your endpoint has no health route** — most hosted services don't, and probing a missing route made the manager treat the endpoint as down and fall back to hashing without ever calling it. Health-path config applies to remote endpoints only; the bundled loopback server is always probed on its own `/health`, because adopting one that is still loading would 503 every call |
| `persistSessionMappings` | `false` | Mirror this session's token-to-value table to disk. Off for the same reason as `auditLogIncludeValues`: the file holds every detected secret in plain text. It used to be written unconditionally, which meant a tool built to keep secrets off the wire accumulated them on disk instead — and bought nothing, since the session id is fresh per activation so the file was never read back. Old files are purged on activation |
| `embeddingApiAuthToken` | `""` | **Deprecated.** A setting is plain text, shown in the Settings UI, copied to other machines by Settings Sync, and committed if set at workspace level. Run **Vaultline: Set Embedding API Token** to keep it in the OS keychain instead. Still read as a fallback when the keychain is empty, so existing installs keep working |
