...
| `embeddingApiUrl` / `embeddingApiTimeoutMs` / `embeddingApiModel` / `embeddingApiAuthType` / `embeddingApiAuthToken` / `embeddingApiKeyHeader` | — | Only relevant when `embeddingBackend` is `"api"`. Point the URL at a remote/OpenAI-compatible endpoint to use one instead of the bundled server (which also disables auto-start) |
